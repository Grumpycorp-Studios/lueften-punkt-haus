...
namespace Lueften {
  ...
  function parseMeasurement(value: string, measurement: Measurement): number {
    // Check presence
    if (!value) {
      return NaN;
    }
    ...
    function rescaleExteriorHumidityToInteriorTemperature(
      ...
      return (
        ((exteriorHumidity *
          Math.pow(
            10,
            (exteriorCoefficients.a * exteriorTemperature) / (exteriorCoefficients.b + exteriorTemperature),
          )) /
          Math.pow(
            10,
            (interiorCoefficients.a * interiorTemperature) / (interiorCoefficients.b + interiorTemperature),
          )) *
